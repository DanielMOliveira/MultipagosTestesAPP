...
class Extra_Checkout_Fields_For_Brazil {
	...
	private static function single_activate() {
		$default = array(
			...
			'ie'              => 0,
			'rg'              => 0,
			//Criado para campo extra no checkout
			'fantasia'        => 0,
			//'birthdate_sex'   => 0,
			'cell_phone'      => 1,
			'mailcheck'       => 1,
	...
	public function load_order_data( $data ) {
		...
		$data['billing_cnpj']          = '';
		$data['billing_ie']            = '';
		//Crinado função publica billing fantasia
		$data['billing_fantasia']	   = '';
	...
	//	}
			   ...
			   // Billing Fantasia.
			$new_fields['billing_fantasia'] = array(
				'label'       => __( 'Nome Fantasia', self::$plugin_slug ),
				'placeholder' => _x( 'Nome Fantasia', 'placeholder', self::$plugin_slug ),
				'class'       => array( 'form-row-first' ),
				'clear'       => false,
				'required'    => true
			);
	...
	public function valid_checkout_fields() {
		...
		if ( isset( $settings['person_type'] ) ) {
			...
			//dentro do primeiro if estava 2 mudei para 1 afim de não exigir required em cima de company -  jeferson luiz 11/04/2014
			if ( 1 == $_POST['billing_persontype'] ) {
				...
				//linha comentada, pois gerava erro na passagem de valor de CPF no momento do cadastrado envia valor de CNPJ na validação 08/05/2014 - jeferson luiz
	...
	public function customer_details_ajax( $customer_data ) {
		...
		$custom_data = array(
			...
			$type_to_load . '_persontype' => get_user_meta( $user_id, $type_to_load . '_persontype', false ), //alterei para false afim de confirmar required false em company  - jeferson luiz 11/04/2014
			$type_to_load . '_cpf' => get_user_meta( $user_id, $type_to_load . '_cpf', false ),
			$type_to_load . '_rg' => get_user_meta( $user_id, $type_to_load . '_rg', false ),
			$type_to_load . '_cnpj' => get_user_meta( $user_id, $type_to_load . '_cnpj', false ),
			$type_to_load . '_ie' => get_user_meta( $user_id, $type_to_load . '_ie', false),
			$type_to_load . '_fantasia' => get_user_meta( $user_id, $type_to_load . '_fantasia', true ),
