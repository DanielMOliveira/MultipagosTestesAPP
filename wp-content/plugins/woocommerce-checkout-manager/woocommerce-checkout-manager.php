...
<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">
	 ...
	 <td><input name="wccs_settings[checkness][wccs_rq_3_s]" type="checkbox" class="rq_s" value="1" <?php if (  1 == ($options['checkness']['wccs_rq_3_s'])) echo "checked='checked'"; ?> /></td>
	 <td></td>
...
<th style="width:5%;" class="wccs-order" title="<?php esc_attr_e( 'Change order' , 'woocommerce-checkout-manager' ); ?>"></th>
		...
		<th><?php _e('Option C' , 'woocommerce-checkout-manager' ); ?></th>
                <th><?php _e('Options D' , 'woocommerce-checkout-manager' ); ?></th>
                <th><?php _e('Options E' , 'woocommerce-checkout-manager' ); ?></th>
                ...
                <th width="4%"><?php _e('Required Attribute' , 'woocommerce-checkout-manager' ); ?></th>
...
<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">
          ...
          value="<?php echo esc_attr( $options['buttons'][$i][option_a] ); ?>" /></td>
           <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][option_b]"
            ...
            <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][option_d]" 
            value="<?php echo esc_attr( $options['buttons'][$i][option_d] ); ?>" /></td>
          <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][option_e]"  
          value="<?php echo esc_attr( $options['buttons'][$i][option_e] ); ?>" /></td>
<td>
<select name="wccs_settings[buttons][<?php echo $i; ?>][type]" >  <!--Call run() function-->
     ...
     <option value="radio" <?php selected( $options['buttons'][$i][type], 'radio' ); ?>>Botão Rádio</option>     
</select>
</td>
...
<input type="hidden" name="<?php echo $hidden_field_name; ?>" value="Y">
       ...
       <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][option_d]" 
       title="<?php esc_attr_e( 'Insert Option D', 'woocommerce-checkout-manager' ); ?>" value="" /></td>
       <td><input type="text" name="wccs_settings[buttons][<?php echo $i; ?>][option_e]" 
       title="<?php esc_attr_e( 'Insert Option E', 'woocommerce-checkout-manager' ); ?>" value="" /></td>
<td>
<select name="wccs_settings[buttons][<?php echo $i; ?>][type]" >  <!--Call run() function-->
     ...
     <option value="radio" >Botão Rádio</option>   
</select>
</td>
...
add_filter( 'woocommerce_checkout_fields' , 'custom_replace_checkout_fields' );//retirei um a do final
// ========== Last Name ==============
...
// Linhas 629 a 633 criadas para função de required false em company - Jeferson Luiz 11/04/2014
...
if (  1 == ($options['checkness']['wccs_rq_8'] ) ) 
	$address_fields8['billing_state']['required'] = false;
...
if ( ! empty( $options['replace']['label2'] ) ) 
     $fields['billing']['billing_state']['label'] = ''.$options['replace']['label2'].'';
     return $fields;
}
...
if ( ! empty( $btn['label'] ) &&  ($btn['type'] == 'select') ) {
   woocommerce_form_field( ''.$btn['cow'].'' , array(
        ...
        'options'     => array(
             ...
             ''.$btn['option_d'].'' => __(''.$btn['option_d'].'', 'woocommerce-checkout-manager' ),
             ''.$btn['option_e'].'' => __(''.$btn['option_e'].'', 'woocommerce-checkout-manager' )
                                                 ),
        'required'  => $btn['checkbox'],
        'placeholder'       => ''.$btn['placeholder'].'',
        ), $checkout->get_value( ''.$btn['cow'].'' )); 
...
//funcção adicionada jeferson luiz 11/04/2014
